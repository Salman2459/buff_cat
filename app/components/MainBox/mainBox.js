        ...
        <div className="lg:w-[900px] py-5 box-border bg-[#301B00E5] m-auto rounded-lg mt-10">
          <div className="w-full md:w-[95%] h-[60px] bg-gradient-to-r from-[#EFCB97] to-[#F3933F]  rounded-lg m-auto ">
            <ol className="flex justify-around items-center h-full font-medium cursor-pointer text-[.7em] sm:text-[.8em] lg:text-[1em]">
              <li
                className={newroute == "Dashboard" ? "font-semibold" : ""}
                onClick={() => setnewroute("Dashboard")}
              >
                Dashboard
              </li>
              <li
                className={newroute == "Localtoken" ? "font-semibold" : ""}
                onClick={() => setnewroute("Localtoken")}
              >
                Lock Token
              </li>
              <li
                className={newroute == "CoinReward" ? "font-semibold" : ""}
                onClick={() => setnewroute("CoinReward")}
              >
                Coin Reward
              </li>
              <li
                className={newroute == "Localtoken" ? "font-semibold" : ""}
                onClick={() => setnewroute("Leaderboard")}
              >
                Leaderboard
              </li>
              <li onClick={() => setnewroute("trendingtoken")}>
                Trending Token
              </li>
            </ol>
          </div>
          ...
          {newroute == "Dashboard" ? (
            <Dashboard />
          ) : newroute == "Leaderboard" ? (
            <Leaderboard />
          ) : newroute == "CoinReward" ? (
            <Coinreward />
          ) : newroute == "trendingtoken" ? (
            <TrendingToken />
          ) : (
            <Locktoken />
          )}
