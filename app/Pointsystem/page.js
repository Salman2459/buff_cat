...
const BoxinBOx = ({ number, head1, head2, para1, para2 }) => {
  return <div className="bg-[#2a1a12] text-white rounded-lg p-5 w-80 shadow-md font-sans mt-10  border-b border-[#F3933F] min-h-[300px] ml-2 mr-2 lg:ml-10 lg:mr-5" >
    ...
    <div className="flex items-center">
      <div>
        <h3 className="text-[1.3em] font-semibold mt-5">{head1}</h3>
        <p className="text-[.9em] text-[#d3c7bb] mt-3 ">
          {para1}
        </p>
      </div>
    </div>
    {
      number < 3 ? <div className="bg-[#3e2d23] rounded-md p-4 mt-4 text-sm">
      <strong className='text-[1.1em] text-[#F3933F]'>{head2}</strong>
      <p className="mt-2 text-[#f1e7d8] text-[1.1em]">{para2}</p>
    </div> : null
    }
  </div>
}
